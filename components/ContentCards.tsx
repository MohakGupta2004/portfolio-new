...
const cardsData: CardType[] = [
  ...
  },
  {
    ...
    author: '- Daredevil',
    // Using a more standard aspect ratio image for better fit initially
    imageUrl: 'https://i.pinimg.com/736x/ef/83/9e/ef839e518f7f3f586fac4dc6cd99d948.jpg',
  ...
  },
  {
    ...
    author: '- Gandalf',
    imageUrl: 'https://i.pinimg.com/736x/32/7b/0a/327b0ab01e8c95a2890a82b8a7f1a308.jpg',
  ...
  },
  {
    ...
    author: '- Yoda',
    imageUrl: 'https://i.pinimg.com/736x/91/b6/20/91b62095f9d7899b8f2761f265691e4a.jpg',
...
export function ContentCards({ index }: ContentCardsProps) {
  ...
  return (
    ...
    >
      <Card
        className={`relative overflow-hidden backdrop-blur-sm border border-red-300/20 p-4 transition-all duration-300 ${card.gradient} ${card.hoverGradient} ${card.type === 'music' ? 'cursor-pointer' : ''}`}
        onClick={card.type === 'music' ? () => window.open(card.spotifyUrl, '_blank') : undefined}
      >
        {/* Aspect ratio box for image */}
        <div className="relative w-full" style={{ paddingBottom: '120%' }}> {/* Adjust paddingBottom for desired aspect ratio */}
          <img
            src={card.imageUrl}
            alt={card.title || 'Card image'}
            className="absolute inset-0 w-full h-full object-cover opacity-80" // object-cover to fill the aspect ratio box
          />
        </div>
        {/* Overlay gradient */}
        ...
        {/* Content */}
        <div className="relative flex flex-col justify-end p-4 z-10" style={{ minHeight: '8rem' }}> {/* minHeight to ensure content area */}
          {card.type === 'music' ? (
            <>
